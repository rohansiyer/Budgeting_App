...
- **Carryover conservation law**: roll/borrow entries come in pairs (shared `pairId`, equal amounts); sweeps also write a real transfer to the savings account in the same transaction. Borrow caps: next week only, ≤50% of its configured budget. `attributionMonth` implements the duck guard (cross-month borrows punish the origin month).
